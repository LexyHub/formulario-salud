...
export default function CellphoneInput({
  ...
  const [number, setNumber] = useState<string>(""); // raw digits only
  const changeRef = useRef(onChange);

  useEffect(() => {
    if (dialCode) {
      const formatted = number ? formatPhone(number) : "";
      changeRef.current(`${dialCode}${formatted ? ` ${formatted}` : ""}`);
    }
  }, [dialCode, number]);
      ...
      <div className='grid grid-cols-[auto_1fr] w-full space-x-2'>
        ...
        <div
          ...
          <input
            ...
            className='bg-transparent outline-none placeholder:text-lexy-text-secondary font-archivo text-sm md:text-base leading-5 w-full'
            id={id}
